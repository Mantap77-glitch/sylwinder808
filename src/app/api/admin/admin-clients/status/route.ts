...
export async function PATCH(req: Request) {
  try {
    ...
    const result = await serverApi(API_ENDPOINTS.admin.adminClientStatus, {
      method: "PATCH",
      token: session.token,
      body,
    });
    ...
    console.error("UPDATE_ADMIN_CLIENT_STATUS_PROXY_ERROR:", error);

    return NextResponse.json(
      {
        success: false,
        message: getBackendMessage(error, "Gagal mengubah status client."),
      },
